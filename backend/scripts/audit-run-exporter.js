...
exports.exportAuditRun = exportAuditRun;
exports.exportMultipleRuns = exportMultipleRuns;
exports.listAvailableRuns = listAvailableRuns;
exports.getExportStats = getExportStats;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
// Dummy data generator
function generateDummyAuditRun(runId) {
    ...
    const totalDeviation = results.reduce((sum, result) => sum + result.deviation, 0);
    const criticalFlags = results.filter((result) => result.deviationType === 'critical').length;
    return {
        id: runId,
        reviewer: 'admin-1',
...
async function fetchAuditRun(runId) {
...
function convertToCSV(auditRun) {
    const csvRows = auditRun.results.map((result) => ({
    ...
    // Generate CSV rows
    const csvData = csvRows.map((row) => headers
        .map((header) => {
...
function convertToJSON(auditRun, options) {
...
function ensureExportsDirectory(outputDir) {
...
async function exportAuditRun(runId, format, options = {}) {
    const { outputDir = 'exports', includeMetadata = true, flattenResults = false, } = options;
...
async function exportMultipleRuns(runIds, format, options = {}) {
    console.log(`🔄 Batch exporting ${runIds.length} audit runs (${format.toUpperCase()})`);
    const results = [];
...
async function getExportStats(outputDir = 'exports') {
    if (!fs.existsSync(outputDir)) {
        return {
            totalFiles: 0,
            csvFiles: 0,
...
//# sourceMappingURL=audit-run-exporter.js.map